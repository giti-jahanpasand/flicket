...
def get_categories():
    ...
    if department:
        categories = categories.filter(
            FlicketCategory.department.has(FlicketDepartment.department == department)
        )
    page = request.args.get("page", 1, type=int)
    per_page = min(
        request.args.get("per_page", app.config["posts_per_page"], type=int), 100
    )
    data = FlicketCategory.to_collection_dict(
        categories, page, per_page, "bp_api.get_categories"
    )
    return jsonify(data)
...
def create_category():
    ...
    if (
        FlicketCategory.query.filter_by(category=data["category"])
        .filter_by(department_id=data["department_id"])
        .first()
    ):
