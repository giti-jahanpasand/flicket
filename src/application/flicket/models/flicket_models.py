...
class FlicketTicket(PaginatedAPIMixin, Base):
    ...
    last_updated = db.Column(
        db.DateTime(), server_default=datetime.datetime.now().strftime("%Y-%m-%d")
    )
    ...
    def my_subscribed_tickets(ticket_query):
        ...
        return ticket_query.filter(
            FlicketTicket.subscribers.any(FlicketSubscription.user_id == g.user.id)
        )
    ...
    def sorted_tickets(ticket_query, sort):
        ...
        if sort == "priority":
            ...
            ticket_query = ticket_query.order_by(
                FlicketTicket.last_updated, FlicketTicket.id
            )
...
class FlicketSubscription(PaginatedAPIMixin, Base):
    ...
    user_def = db.deferred(
        db.select(FlicketUser.name).where(FlicketUser.id == user_id).scalar_subquery()
    )
...
class FlicketAction(PaginatedAPIMixin, Base):
    ...
    def output_action(self):
        ...
        if self.action == "assign":
            return (
                f'Ticket assigned to <a href="mailto:{recipient_name}">{recipient_name}</a>'
                f' by <a href="mailto:{user_email}">{user_name}</a> | {_date}'
            )
        ...
        if self.action == "subscribe":
            return (
                f'<a href="mailto:{recipient_email}">{recipient_name}</a> has been subscribed to ticket'
                f' by <a href="mailto:{user_email}">{user_name}</a>. | {_date}'
            )

        if self.action == "unsubscribe":
            return (
                f'<a href="mailto:{recipient_email}">{recipient_name}</a> '
